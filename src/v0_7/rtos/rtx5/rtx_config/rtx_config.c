...
__WEAK uint32_t osRtxErrorNotify (uint32_t code, void *object_id) {
  ...
  os_thread_t *thread = (os_thread_t *)object_id;
  switch (code) {
  ...
  ASSERT(0, "osRtxErrorNotify, code: %08x object is %p, magicWord:0x%08x|0x%08x\n",
        code, object_id, *((uint32_t *)thread->stack_mem), osRtxStackMagicWord);
//return 0U;
}
