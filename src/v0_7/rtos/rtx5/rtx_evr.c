...
__WEAK void EvrRtxMemoryAlloc (void *mem, uint32_t size, uint32_t type, void *block) {
  ...
  (void)block;
  if(block == NULL)
  ASSERT(0,"RtxMemoryAlloc fail try change OS_DYNAMIC_MEM_SIZE size");
#endif
}
#endif
...
__WEAK void EvrRtxMemoryBlockAlloc (osRtxMpInfo_t *mp_info, void *block) {
  ...
  (void)block;
  if(block==NULL)
  ASSERT(0,"RtxMemoryBlockAlloc fail try change OS_DYNAMIC_MEM_SIZE size");
#endif
}
#endif
...
__WEAK void EvrRtxMemoryPoolError (osMemoryPoolId_t mp_id, int32_t status) {
  ...
  (void)status;
#endif
    ASSERT(0, "EvrRtxMemoryPoolError id:%p status:%d", mp_id, status);
}
#endif
