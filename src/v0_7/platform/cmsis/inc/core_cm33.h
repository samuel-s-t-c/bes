...
void nuttx_irq_attach(IRQn_Type IRQn, uint32_t vector);
bool nuttx_irq_check(IRQn_Type IRQn, bool isEnable);
__STATIC_INLINE void tmp_irq_setPriority(IRQn_Type IRQn, uint32_t pri)
{
...
__STATIC_INLINE void tmp_irq_SetVector(IRQn_Type IRQn, uint32_t vector)
{
#if defined(CONFIG_SMP) || (RAMCP_SIZE == 0)
  nuttx_irq_attach(IRQn, vector);
#else
  if (get_cpu_id() == 1)
