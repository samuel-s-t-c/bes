...
#include "hal_trace.h"
#define MYDEBUG_APP_AI_TWS_EN
#if defined(MYDEBUG_APP_AI_TWS_EN) && defined(MYDEBUG)
#define MYDEBUG_APP_AI_TWS
#endif

#ifdef MYDEBUG_APP_AI_TWS
#define MYTRACE_APP_AI_TWS(attr,str,...) MYTRACE(attr, str, ##__VA_ARGS__)
#else
#define MYTRACE_APP_AI_TWS(attr, str, ...)
#endif
...
static void app_ai_role_switch_timeout_cb(void const *n)
{
	MYTRACE_APP_AI_TWS(1, "Entering %s", __func__);
    app_ai_tws_role_switch_dis_ble();
	MYTRACE_APP_AI_TWS(1, "Leaving %s", __func__);
}
